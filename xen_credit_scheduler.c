...
#define CSCHED_DEFAULT_WEIGHT       256    //默认weight
#define CSCHED_TICKS_PER_TSLICE     3      //每个time slice提醒3次，每10ms一次
#define CSCHED_DEFAULT_TSLICE_MS    30
#define CSCHED_CREDITS_PER_MSEC     10     //每ms扣10，每次10ms也就是扣100credit
...
#define CSCHED_PRIV(_ops)   ((struct csched_private *)((_ops)->sched_data))
#define CSCHED_PCPU(_c)     ((struct csched_pcpu *)per_cpu(schedule_data, _c).sched_priv)
...
//perfc_incr()定义：#define perfc_incr(x)     (++this_cpu(perfcounters)[ PERFC_ ## x])
//The perf counters provide generic internal infrastructure for gauges and counters. The counted 
//values can be both integer and float.
//性能计数器计和计数器提供通用的内部基础设施。计数值既可以是整数，浮点数。还有一个“平均”类型（通常是浮动），
//结合了一个num的计数器，可以分为以提供平均。这些数据将被收集和汇总由工具collectd 和 statsd等，并送入graphite
//之类的工具进行绘图和分析。
...
2, 数据结构
//物理cpu的调度信息，它主要是就绪的vcpu队列，ticker计时器，用于每个tick周期计算当前运行的vcpu的credit值。
struct csched_pcpu {
...
//vcpu的调度信息，它位于pcpu的就绪队列上，也位于活动vcpu列表上，可以得到它所属的domain的调度信息，
//可以得到它的credit值与优先级
struct csched_vcpu {
...
//domain的调度信息，例如活动的vcpu列表，活动domain列表，domain的weight和cap值。
struct csched_dom {
...
//credit调度算法的总体信息，例如调度锁，cpu个数，主ticker计时器，用于每3个tick周期计算一次所有的credit值，
//当前总的weight值和credit值，
struct csched_private {
    ...
    uint32_t ncpus;
    struct timer  master_ticker;    //主ticker的master_timer由csched_priv持有
    unsigned int master;
...
csched_vcpu_acct(struct csched_private *prv, unsigned int cpu)
{
     ...
     */
    //当活动vcpu列表为空时说明所有vcpu的credit都为负，则重新计算credit值。否则通过pick后pcpu是否变化
    //来检查是否需要进行vcpu的迁移，如果需要则触发软中断，执行schedule算法。
    if ( list_empty(&svc->active_vcpu_elem) )
    {
...
//vcpu的初始化主要是把vcpu插入到其pcpu的运行就绪队列runq上和活动vcpu列表中, 在INIT_LIST_HEAD处
static void *
...
__runq_insert(unsigned int cpu, struct csched_vcpu *svc)
{
    ...
    /*find a position in runqueue for vcpu svc*/
    ...
    /*add svc before the element iter*/
...
//在初始化每个pcpu时会设置它的tick的timer，每个tick周期（10ms）会执行一次csched_tick，来更新该pcpu上
//运行的当前vcpu的credit值，减去credit_per_tick，该值是100。当活动vcpu列表为空时说明所有vcpu的credit
//都为负，则重新计算credit值。否则通过pick后pcpu是否变化来检查是否需要进行vcpu的迁移，如果需要则触发软中
//断，执行schedule算法。
static void *
...
//Pick from online CPUs in VCPU's affinity mask
static int
...
//对domain control的操作，比如修改domain weight，cap等等
static int
csched_dom_cntl(
...
//它会计算当前vcpu使用了多少credit之类的accounting, 并把每个runq重新排序
static void
...
9, 总的
//通过csched_acct来计算所有domain的vcpu的credit值，主要是根据当前credit总值，每个domain占用的权重weight和
//比例上限cap，计算出每个domain分得的credit值，然后再平均分配到每个vcpu上，vcpu的credit值就等于原有的credit
//值和新分到的credit之和。根据vcpu最新的credit值来确定vcpu的调度优先级是over还是under，如果是over且cap有设
//置，则执行sleep，把自己调度出来，等待下次判断和调度。


//主timer csched_acct, 定期去计算当前活动dom的活动vcpu里面的credit值，根据一定的策略如weight和cap来决定每
//个vcpu最后分到的credit值，如果当前credit小于0，那么表示credit不够了，优先级要降低，prio设置成over，如果有
//设置cap，那么会pause住，等到后面有credit再唤醒。如果credit > 0那么，优先级就提高了，prio设置为under, 如果
//之前因为cap而被pause的，这个时候可以unpause起来，最后通知说，我需要runq重新排序
static void
 ...
 */
//先去取当前cpu 的runq的下一个任务，取出来之后判断下当前的pri，如果是over，那表示它的credit已经用完了，
//不能被run了，需要重新挑一个出来，如果大于over，表示可以run， 就会从runq里面摘出来。
static struct task_slice
csched_schedule(
    const struct scheduler *ops, s_time_t now, bool_t tasklet_work_scheduled)
{
    const int cpu = smp_processor_id();   //获取当前物理cpu
